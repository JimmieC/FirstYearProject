					...
					foreach($event_length[$thisday] as $i=>$el) {
						...
						switch ($el['state']) {
							case 'begin':
								...
								$event_calno = (($event_calno - 1) % $phpiCal_config->unique_colors) + 1;
								$confirmed = '';
						  		if (is_array($event_recur)) $confirmed .= '<img src="phpicalendar-2.4/images/recurring.gif" width="9" height="9" alt="" border="0" hspace="0" vspace="0" />&nbsp;';
								if ($event_status != '') {
						  			$confirmed .= '<img src="phpicalendar-2.4/images/'.$event_status.'.gif" width="9" height="9" alt="" border="0" hspace="0" vspace="0" />&nbsp;';
						  		}
								...
								if ($event_date_start != $event_date_end) {
									if ($thisday == $event_date_start) {
										$confirmed .= '<img src="phpicalendar-2.4/images/longevent_start.gif" width="9" height="9" alt="" border="0" hspace="0" vspace="0" />&nbsp;';
									}
									else if ($thisday == $event_date_end) {
										$confirmed .= '<img src="phpicalendar-2.4/images/longevent_end.gif" width="9" height="9" alt="" border="0" hspace="0" vspace="0" />&nbsp;';
									}
									else {
										$confirmed .= '<img src="phpicalendar-2.4/images/longevent_mid.gif" width="9" height="9" alt="" border="0" hspace="0" vspace="0" />&nbsp;';
									}
								}
				...
				for ($i=0;$i<sizeof($event_length);$i++) {
					...
					switch ($event_length[$i]['state']) {
						case 'begin':
						  ...
						  $confirmed = '';
						  if (is_array($event_recur)) $confirmed .= '<img src="phpicalendar-2.4/images/recurring.gif" width="9" height="9" alt="" border="0" hspace="0" vspace="0" />&nbsp;';
						  if ($event_status != '') $confirmed .= '<img src="phpicalendar-2.4/images/'.$event_status.'.gif" width="9" height="9" alt="" border="0" hspace="0" vspace="0" />&nbsp;';
						  $event_date_start = date('Ymd', $this_time_arr[$uid]['start_unixtime']);
				...
				if ($type != 'small') {
					foreach ($master_array[$daylink] as $cal_time => $event_times) {
						foreach ($event_times as $uid => $val) {
							...
							if ($cal_time == -1) {
								if ($type == 'large') {
									$switch['ALLDAY'] .= '<div class="V10"><img src="phpicalendar-2.4/templates/'.$phpiCal_config->template.'/images/monthdot_'.$event_calno.'.gif" alt="" width="9" height="9" border="0" />';
 									$switch['ALLDAY'] .= openevent($daylink, $cal_time, $uid, $val, $phpiCal_config->month_event_lines, 15, 'psf');
 									$switch['ALLDAY'] .= $location.'</div>';
								} else {
									$switch['ALLDAY'] = '<img src="phpicalendar-2.4/templates/'.$phpiCal_config->template.'/images/allday_dot.gif" alt=" " width="11" height="10" border="0" />';
								}
							} else {
								$start2		 = date($timeFormat_small, $val['start_unixtime']);
								if ($type == 'large') {
									$switch['EVENT'] .= '<div class="V9"><img src="phpicalendar-2.4/templates/'.$phpiCal_config->template.'/images/monthdot_'.$event_calno.'.gif" alt="" width="9" height="9" border="0" />';
									...
									$switch['EVENT'] = '<img src="phpicalendar-2.4/templates/'.$phpiCal_config->template.'/images/event_dot.gif" alt=" " width="11" height="10" border="0" />';
								}
							}
						}
					}
